...
import (
	...
	ojosim "github.com/cheqd/cheqd-node/util/sim"
...
var (
	acceptList = []string{types.CheqdSymbol, types.USDSymbol}
...
func GenerateExchangeRatesString(prices map[string]math.LegacyDec) string {
...
func SimulateMsgAggregateExchangeRatePrevote(
	...
	return func(
		...
		prices := make(map[string]math.LegacyDec, len(acceptList))
		for _, denom := range acceptList {
			prices[denom] = ojoPrice.Add(simtypes.RandomDecAmount(r, math.LegacyNewDec(1)))
		}
...
func deliver(r *rand.Rand, app *baseapp.BaseApp, ctx sdk.Context, ak simulation.AccountKeeper,
	...
	return ojosim.GenAndDeliver(bk, o, 200000*50) // TODO: replace 10 to default Gas limit
}
