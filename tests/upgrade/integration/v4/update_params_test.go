...
var _ = Describe("Integration - update the did params", func() {
	...
	It("should generate update-did-fee-params.json with usd.min = ncheq.min * price", func() {
		...
		didParams, err := cli.QueryDidFeeParams(cli.Validator0)
		Expect(err).To(BeNil())
	...
	It("should submit a update param proposal ", func() {
		...
		res, err := cli.SubmitProposal(cli.Validator0, "update-did-fee-params.json")
		Expect(err).To(BeNil())
	...
	It("should vote for the update param proposal from `validator1` container", func() {
	...
	It("should vote for the update param proposal from `validator2` container", func() {
	...
	It("should vote for the update param proposal from `validator0` container", func() {
...
var _ = Describe("Integration - update resource params", func() {
	...
	It("should generate update-resource-fee-params.json with usd.min = ncheq.min * price", func() {
		By("querying current Resource fee params")
		resourceParams, err := cli.QueryResourceFeeParams(cli.Validator0)
		Expect(err).To(BeNil())
	...
	It("should submit a update param proposal", func() {
		...
		By("sending a SubmitProposal transaction from `validator0` container")
		res, err := cli.SubmitProposal(cli.Validator0, "update-resource-fee-params.json")
		Expect(err).To(BeNil())
		res, err = cli.QueryTxn(cli.Validator0, res.TxHash)
		Expect(err).To(BeNil())
