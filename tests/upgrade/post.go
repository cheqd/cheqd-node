...
func Post() error {
	...
	res, err = cli.QueryDid(PostDidDoc.Id, cli.VALIDATOR0)
	Expect(err).To(BeNil())
	Expect(res.Did.Id).To(BeEquivalentTo(PostRotatedKeysDidDoc.Id))
	Expect(res.Did.Controller).To(BeEquivalentTo(PostRotatedKeysDidDoc.Controller))
	Expect(res.Did.VerificationMethod).To(HaveLen(1))
	Expect(res.Did.VerificationMethod[0].Id).To(BeEquivalentTo(PostRotatedKeysDidDoc.VerificationMethod[0].Id))
	Expect(res.Did.VerificationMethod[0].Type).To(BeEquivalentTo(PostRotatedKeysDidDoc.VerificationMethod[0].Type))
	Expect(res.Did.VerificationMethod[0].Controller).To(BeEquivalentTo(PostRotatedKeysDidDoc.VerificationMethod[0].Controller))
	Expect(res.Did.VerificationMethod[0].PublicKeyMultibase).To(BeEquivalentTo(PostRotatedKeysDidDoc.VerificationMethod[0].PublicKeyMultibase))
	Expect(res.Did.Authentication).To(HaveLen(1))
