...
from helpers import random_string, wallet_helper, get_base_account_number_and_sequence, get_timeout_height, get_balance_vdr, send_tx_helper, \
...
@pytest.mark.parametrize("memo", ["a", "test_memo_test", "123qwe$%^&", "______________________________"])
@pytest.mark.asyncio
...
async def test_did_positive():
    ...
    res, tx_hash = await send_tx_helper(pool_alias, wallet_handle, key_alias, public_key, SENDER_ADDRESS, bytes(signed_req), default_memo)
