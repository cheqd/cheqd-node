...
@pytest.mark.parametrize(
        ...
        [
            ...
            ("status", LOCAL_NET_NODE_TCP, fr"\"NodeInfo\"(.*?)\"network\":\"{LOCAL_NET_NETWORK}\"(.*?)\"moniker\":\"validator-0\""), # tcp + local
            ("status", LOCAL_NET_NODE_HTTP, fr"\"NodeInfo\"(.*?)\"network\":\"{LOCAL_NET_NETWORK}\"(.*?)\"moniker\":\"validator-0\""), # http + local
        ]
    )
...
def test_keys_import_wrong_data(action, expected_result):
    ...
    run(command_base, "add", f"{key_name} {KEYRING_BACKEND_TEST}", "name: {}".format(key_name))

    cli = run(command_base, "export", f"{key_name} {KEYRING_BACKEND_TEST}", "Enter passphrase")
    run_interaction(cli, passphrase, "BEGIN")
...
@pytest.mark.parametrize(
        ...
        [
            ...
            ("staking create-validator", fr"--amount 1000000000000000ncheq --from {set_up_operator()[0]} --pubkey {set_up_operator()[2]} --min-self-delegation='1' --commission-max-change-rate='0.02' --commission-max-rate='0.02' --commission-rate='0.01' {TEST_NET_FEES} {LOCAL_NET_DESTINATION} {YES_FLAG} {KEYRING_BACKEND_TEST}", r"\"code\":6(.*?)validator pubkey type is not supported"), # wrong pubkey type
            ("staking create-validator", fr"--amount 1000000000000000ncheq --from {set_up_operator()[0]} --pubkey '{NODE_PUBKEY}' --min-self-delegation='1' --commission-max-change-rate='0.02' --commission-max-rate='0.02' --commission-rate='0.01' {TEST_NET_FEES} {LOCAL_NET_DESTINATION} {YES_FLAG} {KEYRING_BACKEND_TEST}", fr"{CODE_0}"),
            ("staking create-validator", fr"--amount 1000000000000000ncheq --from {set_up_operator()[0]} --pubkey '{NODE_PUBKEY}' --min-self-delegation='1' --commission-max-change-rate='0.02' --commission-max-rate='0.02' --commission-rate='0.01' {TEST_NET_FEES} {LOCAL_NET_DESTINATION} {YES_FLAG} {KEYRING_BACKEND_TEST}", fr"\"code\":5(.*?)validator already exist for this pubkey"), # promote twice the same pubkey
        ]
)
...
@settings(deadline=None, max_examples=20) # 20
...
def test_token_transfer(value):
    ...
    run("cheqd-noded tx", "bank send", f"{LOCAL_SENDER_ADDRESS} {LOCAL_RECEIVER_ADDRESS} {value}ncheq {LOCAL_NET_DESTINATION} {TEST_NET_GAS_X_GAS_PRICES} {YES_FLAG} {KEYRING_BACKEND_TEST}", fr"{CODE_0}(.*?)\"value\":\"{value}ncheq\"")
