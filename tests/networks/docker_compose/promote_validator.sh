#!/bin/bash
sudo chown -R runner:docker ${NODE_CONFIGS_BASE}/home/client
cheqd-noded keys list --keyring-backend "test" --home ${NODE_CONFIGS_BASE}/home/client/.cheqdnode/

# Get operator0 address by setting --home flag
OP0_ADDRESS=$(cheqd-noded keys list --keyring-backend "test" --home ${NODE_CONFIGS_BASE}/home/client/.cheqdnode/ | sed -nr 's/.*address: (.*?).*/\1/p' | sed -n 1p | sed 's/\r//g')
...
cheqd-noded tx bank send ${OP0_ADDRESS} ${OP5_ADDRESS} 1100000000000000ncheq --chain-id cheqd --fees 5000000ncheq --node "http://localhost:26657" -y --keyring-backend "test" --home ${NODE_CONFIGS_BASE}/home/client/.cheqdnode/
