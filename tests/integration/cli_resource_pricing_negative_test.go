...
var _ = Describe("cheqd cli - negative resource pricing", func() {
	...
	BeforeEach(func() {
		...
		// Submit the DID Doc
		resp, err := cli.CreateDidDoc(tmpDir, didPayload, signInputs, "", testdata.BASE_ACCOUNT_1, helpers.GenerateFees(didFeeParams.CreateDid[0].MinAmount.String()+didFeeParams.CreateDid[0].Denom))
		Expect(err).To(BeNil())
	...
	It("should not fail in create resource json message - case: fixed fee, lower amount than required", func() {
		...
		lowerTax := sdk.NewCoin(resourceFeeParams.Json[0].Denom, sdkmath.NewInt(resourceFeeParams.Json[0].MinAmount.Int64()-1))
		res, err := cli.CreateResource(tmpDir, resourcetypes.MsgCreateResourcePayload{
			CollectionId: collectionID,
			Id:           resourceID,
			Name:         resourceName,
	...
	It("should not fail in create resource image message - case: fixed fee, lower amount than required", func() {
		...
		lowerTax := sdk.NewCoin(resourceFeeParams.Image[0].Denom, sdkmath.NewInt(resourceFeeParams.Image[0].MinAmount.Int64()-1))
		res, err := cli.CreateResource(tmpDir, resourcetypes.MsgCreateResourcePayload{
			CollectionId: collectionID,
			Id:           resourceID,
			Name:         resourceName,
	...
	It("should not fail in create resource default message - case: fixed fee, lower amount than required", func() {
		...
		lowerTax := sdk.NewCoin(resourceFeeParams.Default[0].Denom, sdkmath.NewInt(resourceFeeParams.Default[0].MinAmount.Int64()-1))
		res, err := cli.CreateResource(tmpDir, resourcetypes.MsgCreateResourcePayload{
			CollectionId: collectionID,
			Id:           resourceID,
			Name:         resourceName,
	...
	It("should not succeed in create resource json message - case: fixed fee, insufficient funds", func() {
		...
		res, err := cli.CreateResource(tmpDir, resourcetypes.MsgCreateResourcePayload{
			...
			Version:      resourceVersion,
			ResourceType: resourceType,
		}, signInputs, resourceFile, testdata.BASE_ACCOUNT_3, helpers.GenerateFees(tax.MinAmount.String()+tax.Denom))
		Expect(err).To(BeNil())
	...
	It("should not succeed in create resource image message - case: fixed fee, insufficient funds", func() {
		...
		res, err := cli.CreateResource(tmpDir, resourcetypes.MsgCreateResourcePayload{
			...
			Version:      resourceVersion,
			ResourceType: resourceType,
		}, signInputs, resourceFile, testdata.BASE_ACCOUNT_3, helpers.GenerateFees(tax.MinAmount.BigInt().String()+tax.Denom))
		Expect(err).To(BeNil())
		Expect(res.RawLog).To(ContainSubstring(sdkerrors.ErrInsufficientFunds.Error()))
	})
	...
	It("should not charge more than tax in create resource json message - case: fixed fee", func() {
		...
		Expect(tax.MaxAmount).NotTo(BeNil())
		Expect(diff).To(Equal(*tax.MaxAmount))
	})
	...
	It("should not charge more than tax in create resource default message - case: fixed fee", func() {
		...
		By("checking that the fee payer account balance has been decreased by the tax")
		diff := balanceBefore.Amount.Sub(balanceAfter.Amount)
		Expect(tax.MaxAmount).ToNot(BeNil())
		Expect(diff).To(Equal(*tax.MaxAmount))
	})
})
