...
var _ = Describe("cheqd cli - negative diddoc pricing", func() {
	...
	It("should not succeed in create diddoc message - case: fixed fee, invalid denom", func() {
		...
		res, err := cli.CreateDidDoc(tmpDir, payload, signInputs, testdata.BASE_ACCOUNT_4, helpers.GenerateFees(invalidTax.String()))
		Expect(err).To(BeNil())
	...
	It("should not succeed in update diddoc message - case: fixed fee, invalid denom", func() {
		...
		res, err := cli.CreateDidDoc(tmpDir, payload, signInputs, testdata.BASE_ACCOUNT_4, helpers.GenerateFees(feeParams.GetCreateDid().String()))
		Expect(err).To(BeNil())
		...
		res, err = cli.UpdateDidDoc(tmpDir, payload2, signInputs, testdata.BASE_ACCOUNT_4, helpers.GenerateFees(invalidTax.String()))
		Expect(err).To(BeNil())
		Expect(res.Code).To(BeEquivalentTo(10))
	})
	...
	It("should not succeed in deactivate diddoc message - case: fixed fee, invalid denom", func() {
		...
		res, err = cli.DeactivateDidDoc(tmpDir, payload2, signInputs, testdata.BASE_ACCOUNT_4, helpers.GenerateFees(invalidTax.String()))
		Expect(err).To(BeNil())
		Expect(res.Code).To(BeEquivalentTo(10))
	})
	...
	It("should not fail in update diddoc message - case: fixed fee, lower amount than required", func() {
		...
		res, err = cli.UpdateDidDoc(tmpDir, payload2, signInputs, testdata.BASE_ACCOUNT_4, helpers.GenerateFees(lowerTax.String()))
		Expect(err).To(BeNil())
		Expect(res.Code).To(BeEquivalentTo(0))
	})
	...
	It("should not fail in deactivate diddoc message - case: fixed fee, lower amount than required", func() {
		...
		res, err = cli.DeactivateDidDoc(tmpDir, payload2, signInputs, testdata.BASE_ACCOUNT_4, helpers.GenerateFees(lowerTax.String()))
		Expect(err).To(BeNil())
		Expect(res.Code).To(BeEquivalentTo(0))
	})
	...
	It("should not succeed in update diddoc message - case: gas auto, insufficient funds", func() {
		...
		By("submitting update diddoc message with insufficient funds")
		res, err = cli.UpdateDidDoc(tmpDir, payload2, signInputs, testdata.BASE_ACCOUNT_3, cli.CLI_GAS_PARAMS)
		Expect(err).To(BeNil())
		Expect(res.Code).To(BeEquivalentTo(5))
	})
	...
	It("should not succeed in deactivate diddoc message - case: gas auto, insufficient funds", func() {
		...
		By("submitting deactivate diddoc message with insufficient funds")
		res, err = cli.DeactivateDidDoc(tmpDir, payload2, signInputs, testdata.BASE_ACCOUNT_3, cli.CLI_GAS_PARAMS)
		Expect(err).To(BeNil())
		Expect(res.Code).To(BeEquivalentTo(5))
	})
	...
	It("should not charge more than tax for create diddoc message - case: fixed fee", func() {
		...
		res, err := cli.CreateDidDoc(tmpDir, payload, signInputs, testdata.BASE_ACCOUNT_4, helpers.GenerateFees(doubleTax.String()))
		Expect(err).To(BeNil())
		Expect(res.Code).To(BeEquivalentTo(0))
	...
	It("should not charge more than tax for update diddoc message - case: fixed fee", func() {
		...
		tax := feeParams.UpdateDid
		doubleTax := sdk.NewCoin(types.BaseMinimalDenom, tax.Amount.Mul(sdk.NewInt(2)))
		res, err = cli.UpdateDidDoc(tmpDir, payload2, signInputs, testdata.BASE_ACCOUNT_4, helpers.GenerateFees(doubleTax.String()))
		Expect(err).To(BeNil())
		Expect(res.Code).To(BeEquivalentTo(0))
	...
	It("should not charge more than tax for deactivate diddoc message - case: fixed fee", func() {
		...
		tax := feeParams.DeactivateDid
		doubleTax := sdk.NewCoin(types.BaseMinimalDenom, tax.Amount.Mul(sdk.NewInt(2)))
		res, err = cli.DeactivateDidDoc(tmpDir, payload2, signInputs, testdata.BASE_ACCOUNT_4, helpers.GenerateFees(doubleTax.String()))
		Expect(err).To(BeNil())
		Expect(res.Code).To(BeEquivalentTo(0))
