...
var _ = Describe("cheqd cli - negative did", func() {
	...
	It("cannot create diddoc with missing cli arguments, sign inputs mismatch, non-supported VM type, already existing did", func() {
		...
		res, err := cli.CreateDidDoc(tmpDir, payload, signInputs, "", testdata.BASE_ACCOUNT_2, helpers.GenerateFees(feeInNCheq.Amount.String()+feeInNCheq.Denom))
		Expect(err).To(BeNil())
		...
		_, err = cli.CreateDidDoc(tmpDir, didcli.DIDDocument{}, []didcli.SignInput{}, "", "", helpers.GenerateFees(feeInNCheq.Amount.String()+feeInNCheq.Denom))
		Expect(err).ToNot(BeNil())

		//   b. missing payload, sign inputs
		_, err = cli.CreateDidDoc(tmpDir, didcli.DIDDocument{}, []didcli.SignInput{}, "", testdata.BASE_ACCOUNT_2, helpers.GenerateFees((feeInNCheq.Amount.String() + feeInNCheq.Denom)))
		Expect(err).ToNot(BeNil())

		//   c. missing payload, account
		_, err = cli.CreateDidDoc(tmpDir, didcli.DIDDocument{}, signInputs2, "", "", helpers.GenerateFees((feeInNCheq.Amount.String() + feeInNCheq.Denom)))
		Expect(err).ToNot(BeNil())

		//   d. missing sign inputs, account
		_, err = cli.CreateDidDoc(tmpDir, payload2, []didcli.SignInput{}, "", "", helpers.GenerateFees((feeInNCheq.Amount.String() + feeInNCheq.Denom)))
		Expect(err).ToNot(BeNil())

		//   e. missing payload
		_, err = cli.CreateDidDoc(tmpDir, didcli.DIDDocument{}, signInputs2, "", testdata.BASE_ACCOUNT_2, helpers.GenerateFees((feeInNCheq.Amount.String() + feeInNCheq.Denom)))
		Expect(err).ToNot(BeNil())

		//   f. missing sign inputs
		_, err = cli.CreateDidDoc(tmpDir, payload2, []didcli.SignInput{}, "", testdata.BASE_ACCOUNT_2, helpers.GenerateFees((feeInNCheq.Amount.String() + feeInNCheq.Denom)))
		Expect(err).ToNot(BeNil())

		//   g. missing account
		_, err = cli.CreateDidDoc(tmpDir, payload2, signInputs2, "", "", helpers.GenerateFees((feeInNCheq.Amount.String() + feeInNCheq.Denom)))
		Expect(err).ToNot(BeNil())
		...
		_, err = cli.CreateDidDoc(tmpDir, payload2, signInputs, "", testdata.BASE_ACCOUNT_2, helpers.GenerateFees((feeInNCheq.Amount.String() + feeInNCheq.Denom)))
		Expect(err).ToNot(BeNil())
		...
		}, "", testdata.BASE_ACCOUNT_2, helpers.GenerateFees((feeInNCheq.Amount.String() + feeInNCheq.Denom)))
		Expect(err).ToNot(BeNil())
		...
		_, err = cli.CreateDidDoc(tmpDir, payload3, signInputs2, "", testdata.BASE_ACCOUNT_2, helpers.GenerateFees((feeInNCheq.Amount.String() + feeInNCheq.Denom)))
		Expect(err).ToNot(BeNil())
		...
		_, err = cli.CreateDidDoc(tmpDir, payload, signInputs, "", testdata.BASE_ACCOUNT_1, helpers.GenerateFees((feeInNCheq.Amount.String() + feeInNCheq.Denom)))
		Expect(err).ToNot(BeNil())
	})
	...
	It("cannot update a DID Doc with missing cli arguments, sign inputs mismatch, non-supported VM type, non-existing did, unchanged payload", func() {
		...
		res, err := cli.CreateDidDoc(tmpDir, payload, signInputs, "", testdata.BASE_ACCOUNT_1, helpers.GenerateFees(feeInNCheq.Amount.String()+feeInNCheq.Denom))
		Expect(err).To(BeNil())
		Expect(res.Code).To(BeEquivalentTo(0))
		...
		res, err = cli.UpdateDidDoc(tmpDir, updatedPayload, signInputs, "", testdata.BASE_ACCOUNT_1, helpers.GenerateFees(fees.String()+feeInNCheq.Denom))
		Expect(err).To(BeNil())
		Expect(res.Code).To(BeEquivalentTo(0))
		...
		feeInNCheq, err = cli.ResolveFeeFromParams(feeParams.CreateDid, useMin)
		Expect(err).To(BeNil())
		res_, err := cli.CreateDidDoc(tmpDir, payload2, signInputs2, "", testdata.BASE_ACCOUNT_2, helpers.GenerateFees(feeInNCheq.Amount.String()+feeInNCheq.Denom))
		Expect(err).To(BeNil())
		...
		feeInNCheq, err = cli.ResolveFeeFromParams(feeParams.UpdateDid, useMin)
		Expect(err).To(BeNil())
		_, err = cli.UpdateDidDoc(tmpDir, didcli.DIDDocument{}, []didcli.SignInput{}, "", "", helpers.GenerateFees(feeInNCheq.Amount.String()+feeInNCheq.Denom))
		Expect(err).ToNot(BeNil())

		//   b. missing payload, sign inputs
		_, err = cli.UpdateDidDoc(tmpDir, didcli.DIDDocument{}, []didcli.SignInput{}, "", testdata.BASE_ACCOUNT_1, helpers.GenerateFees(feeInNCheq.Amount.String()+feeInNCheq.Denom))
		Expect(err).ToNot(BeNil())

		//   c. missing payload, account
		_, err = cli.UpdateDidDoc(tmpDir, didcli.DIDDocument{}, signInputs, "", testdata.BASE_ACCOUNT_1, helpers.GenerateFees(feeInNCheq.Amount.String()+feeInNCheq.Denom))
		Expect(err).ToNot(BeNil())

		//   d. missing sign inputs, account
		_, err = cli.UpdateDidDoc(tmpDir, followingUpdatedPayload, []didcli.SignInput{}, "", testdata.BASE_ACCOUNT_1, helpers.GenerateFees(feeInNCheq.Amount.String()+feeInNCheq.Denom))
		Expect(err).ToNot(BeNil())
		...
		//   g. missing account
		_, err = cli.UpdateDidDoc(tmpDir, followingUpdatedPayload, signInputs, "", "", helpers.GenerateFees(feeInNCheq.Amount.String()+feeInNCheq.Denom))
		Expect(err).ToNot(BeNil())
		...
		_, err = cli.UpdateDidDoc(tmpDir, followingUpdatedPayload, signInputsFuzzed, "", testdata.BASE_ACCOUNT_1, helpers.GenerateFees(feeInNCheq.Amount.String()+feeInNCheq.Denom))
		Expect(err).ToNot(BeNil())

		//   b. sign inputs invalid length
		_, err = cli.UpdateDidDoc(tmpDir, followingUpdatedPayload, signInputs, "", testdata.BASE_ACCOUNT_1, helpers.GenerateFees(feeInNCheq.Amount.String()+feeInNCheq.Denom))
		Expect(err).ToNot(BeNil())
		...
		}, "", testdata.BASE_ACCOUNT_1, helpers.GenerateFees(feeInNCheq.Amount.String()+feeInNCheq.Denom))
		Expect(err).ToNot(BeNil())
		...
		_, err = cli.UpdateDidDoc(tmpDir, invalidVmTypePayload, signInputsAugmented, "", testdata.BASE_ACCOUNT_1, helpers.GenerateFees(feeInNCheq.Amount.String()+feeInNCheq.Denom))
		Expect(err).ToNot(BeNil())
		...
		_, err = cli.UpdateDidDoc(tmpDir, nonExistingDidPayload, signInputsAugmented, "", testdata.BASE_ACCOUNT_1, helpers.GenerateFees(feeInNCheq.Amount.String()+feeInNCheq.Denom))
		Expect(err).ToNot(BeNil())

		// Finally, update the DID Doc

		res, err = cli.UpdateDidDoc(tmpDir, followingUpdatedPayload, signInputsAugmented, "", testdata.BASE_ACCOUNT_1, helpers.GenerateFees(fees.String()+feeInNCheq.Denom))
		Expect(err).To(BeNil())
		Expect(res.Code).To(BeEquivalentTo(0))
		...
		_, err = cli.UpdateDidDoc(tmpDir, followingUpdatedPayload, signInputsAugmented, "", testdata.BASE_ACCOUNT_1, helpers.GenerateFees(fees.String()+feeInNCheq.Denom))
