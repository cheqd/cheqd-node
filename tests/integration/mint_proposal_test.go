...
var _ = Describe("Integration - Mint coins to given address", func() {
	...
	It("should vote for the mint proposal from `validator1` container", func() {
		...
		res, err := cli.VoteProposalTx(cli.Validator1, "1", "yes", cli.CliGasParams)
		Expect(err).To(BeNil())
		Expect(res.Code).To(BeEquivalentTo(0))
	})
	...
	It("should vote for the mint proposal from `validator2` container", func() {
		...
		res, err := cli.VoteProposalTx(cli.Validator2, "1", "yes", cli.CliGasParams)
		Expect(err).To(BeNil())
		Expect(res.Code).To(BeEquivalentTo(0))
	})
	...
	It("should vote for the mint proposal from `validator3` container", func() {
		...
		res, err := cli.VoteProposalTx(cli.Validator3, "1", "yes", cli.CliGasParams)
		Expect(err).To(BeNil())
		Expect(res.Code).To(BeEquivalentTo(0))
	})
	It("should vote for the mint proposal from `validator0` container", func() {
		...
		res, err := cli.VoteProposalTx(cli.Validator0, "1", "yes", cli.CliGasParams)
		Expect(err).To(BeNil())
		Expect(res.Code).To(BeEquivalentTo(0))
	})
