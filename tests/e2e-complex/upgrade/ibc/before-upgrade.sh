...
echo "=> Check balances the second time" # ---
CHEQD_BALANCE_2=$(set +x && localnet_compose exec "${CHEQD_SERVICE}" cheqd-noded query bank balances "$CHEQD_USER_ADDRESS" --output json)
BALANCES=$(ibc_compose exec osmosis osmosisd query bank balances "$OSMOSIS_USER_ADDRESS" --output json)
...
ibc_compose exec osmosis osmosisd query ibc-transfer denom-trace "$DENOM_CUT" > "denom_trace.txt"
