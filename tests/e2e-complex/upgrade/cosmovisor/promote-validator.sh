...
OP2_ADDRESS=$(cheqd-noded keys list --keyring-backend "test" --home "${NODE_CONFIGS_BASE}/client/.cheqdnode" | sed -nr 's/.*address: (.*?).*/\1/p' | sed -n 3p | sed 's/\r//g')
...
cheqd_noded_docker query bank balances "${OP2_ADDRESS}"

local_client_tx tx bank send "${OP2_ADDRESS}" "${OP5_ADDRESS}" 1100000000000000ncheq --chain-id cheqd --fees 5000000ncheq -y
