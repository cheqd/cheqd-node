...
RESULT=$(cheqd-noded tx cheqd create-did "${MSG_CREATE_DID}" "${KEY_ID}" "${OLD_VER_PRIV_BASE_64}" \
...
# Post the message
# shellcheck disable=SC2086
RESULT=$(cheqd-noded tx cheqd update-did "${MSG_UPDATE_DID}" "${KEY_ID}" "${OLD_VER_PRIV_BASE_64}" "${KEY_ID}" "${NEW_VER_PRIV_BASE_64}" \
  --from "${BASE_ACCOUNT_1}" ${TX_PARAMS})
