...
- Get `tar` archive with the binary compiled for Ubuntu 20.04 in [releases](https://github.com/verim-id/verim-node/releases); <-- Recommended
- Get `deb` for Ubuntu 20.04 in [releases](https://github.com/verim-id/verim-node/releases);
- Get docker image form [packages](https://github.com/verim-id/verim-node/pkgs/container/verim-node).
...
2. Get the binary using one of the [described ways](#binary-distribution);
    ...
    ```
    cp verim-noded /usr/bin
    ```

3. Initialize node config files:
        
    Command: `verim-noded init <node_name>`
    
    Example: `verim-noded init alice-node`
