...
# Copy network configuration folder for specific moniker


# Initialise node config directory
if [ ! -d "${CHEQD_NODED_HOME}/config" ]
then
...
then
    ...
    cp /genesis "${CHEQD_NODED_HOME}/config/genesis.json"
else
...
then
    ...
    cp /seeds "${CHEQD_NODED_HOME}/config/seeds.txt"
    CHEQD_NODED_P2P_SEEDS="$(cat "${CHEQD_NODED_HOME}/config/seeds.txt")"
...
then
    ...
    cp /node-key "${CHEQD_NODED_HOME}/config/node_key.json"
else
...
then
    ...
    cp /private-validator-key "${CHEQD_NODED_HOME}/config/priv_validator_key.json"
else
