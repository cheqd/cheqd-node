...
function configure_node() {
  ...
  sed -i $SED_EXT 's/log_level = "error"/log_level = "info"/g' "${CONFIG_TOML}"
...
function configure_genesis() {
  ...
  # Short voting period
  sed -i $SED_EXT 's/"voting_period": "172800s"/"voting_period": "30s"/' "${GENESIS}"
  sed -i $SED_EXT 's/"quorum": "0\.334000000000000000"/"quorum": "0\.10000000000000000"/g' "${GENESIS}"
...
for ((i = 0; i < OBSERVERS_COUNT; i++)); do
